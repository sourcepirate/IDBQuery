            ...
            req.onupgradeneeded = function(event) {
                ...
                if (Schema.hasOwnProperty('length')) {
                    Schema.forEach(function(scheme) {
                        var store = db.createObjectStore(scheme.tabel.name, {
                            keyPath: scheme.tabel.getPrimaryKey().name
                        });
                        scheme.tabel.properties.forEach(function(prop) {
                            if (!prop.key) {
                                store.createIndex(prop.name, prop.name, {
                                    unique: false
                                });
                            } else {
                                store.createIndex(prop.name, prop.name, {
...
DataBase.prototype = {
    ...
    Filter: function(propertyname, condition, value) {
        var values = [];
        var self = this;
        switch (condition) {
            case "eq":
                for (var i = 0; i < self.queryresult.length; i++) {
                    if (self.queryresult[i][propertyname] == value) {
                        values.push(self.queryresult[i]);
                    }
                }
                break;
